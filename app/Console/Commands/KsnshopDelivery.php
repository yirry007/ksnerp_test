...
class KsnshopDelivery extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ksnshop:delivery';
     ...
     * @var string
     */
    protected $description = 'Automatically get ksnshop delivery info';
...
//        $privateIP = '172.26.13.132';
        if (!isValidIP($privateIP)) {
            Log::info('[schedule:run ksnshop:delivery] invalid ip address ".' . $privateIP . '"');
            return false;
        }
        ...
        if (!count($users)) {
            Log::info('[schedule:run ksnshop:delivery] can not find user data.');
            return false;
        }

        foreach ($users as $user) {
            $process = Process::where(['user_id'=>$user->id, 'type'=>'4'])->first();
            /** 跳过更新订单中的用户 */
            ...
            if (!$shop->shop_ids) {
                Log::info('[schedule:run ksnshop:delivery] can not find shop data; user_id: ' . $user->id);
            ...
            $orderItems = OrderItem::with('orderItemMores')->select(['orders.order_id', 'order_items.id', 'order_items.sh_shop_id', 'order_items.item_id', 'order_items.item_options', 'order_items.supply_code', 'order_items.supply_options', 'order_items.supply_order_id'])
                ...
                ->where([
                    ...
                    ['order_items.supply_market', 'ksnshop'],
            ...
            if (!count($orderItems)) {
                Log::info('[schedule:run ksnshop:delivery] can not find order data; user_id: ' . $user->id);
                $this->stopUserProcess($process->id);
                continue;
            }

            $supplier = SupplierInfo::where(['market'=>'Ksnshop', 'user_id'=>$user->id])->first();
            ...
            foreach ($orderItems as $v) {
                ...
                if ($v->order_item_mores) {
                    foreach ($v->order_item_mores as $v1) {
                        /** 获取配送信息 */
                        try {
                            $resultMore = Agent::Supplier($supplier)->setClass('Trade')->getDelivery($v1->supply_order_id);
                        } catch (\Exception $e) {
                        ...
                        $updateResultMore = OrderItemMore::where('id', $v1->id)->update([
                            'supply_delivery_code'=>$resultMore['result']['delivery_code'],
                            'supply_delivery_name'=>$resultMore['result']['delivery_name'],
                            'supply_delivery_number'=>$resultMore['result']['delivery_id'],
                        ]);

                        if ($updateResultMore === false) {
                            Log::info('[schedule:run ksnshop:delivery] ksnshop_order_id:' . $v->supply_order_id . '[more] delivery info update failed.');
                        } else {
                            Log::info('[schedule:run ksnshop:delivery] ksnshop_order_id:' . $v->supply_order_id . '[more] order update success.');
                        }
                    }
                }
                ...
                /** 获取配送信息 */
                try {
                    $result = Agent::Supplier($supplier)->setClass('Trade')->getDelivery($v->supply_order_id);
                } catch (\Exception $e) {
                ...
                $supplyDeliveryNumber = $result['result']['delivery_id'];
                ...
                $updateResult = OrderItem::where('id', $v->id)->update([
                    'supply_delivery_code'=>$result['result']['delivery_code'],
                    'supply_delivery_name'=>$result['result']['delivery_name'],
                    'supply_delivery_number'=>$supplyDeliveryNumber,
                ]);

                if ($updateResult === false) {
                    Log::info('[schedule:run ksnshop:delivery] ksnshop_order_id:' . $v->supply_order_id . ' delivery info update failed.');
                } else {
                    Log::info('[schedule:run ksnshop:delivery] ksnshop_order_id:' . $v->supply_order_id . ' order update success.');
                    ...
                    if ($sourcingFinish) {
                        ...
                        Log::info('[schedule:run ksnshop:delivery] order_id:' . $v->order_id . ' order status has changed to "等待出库".');
                    }
                }
            ...
            Log::info('[schedule:run ksnshop:delivery] ksnshop delivery check finished user_id:' . $user->id);
